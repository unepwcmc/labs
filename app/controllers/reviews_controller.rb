class ReviewsController < ApplicationController
  ...
  def index
    @reviews = Review.includes(:product, :reviewer).order("created_at DESC")
    respond_with(@reviews)
  end

  def new
    @review = Review.new(reviewer_id: current_user.id, product_id: params[:product_id])
  end
  ...
  def review_params
    params.require(:review).permit(:product_id, :reviewer_id)
  end
